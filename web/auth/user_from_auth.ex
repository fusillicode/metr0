defmodule Metr0.UserFromAuth do
  alias Metr0.User
  alias Metr0.Authorization
  alias Guardian.Utils, as: GuardianUtils
  ...
  def get_or_insert(auth, current_user, repo) do
    case auth_and_validate(auth, repo) do
      ...
      authorization ->
        if authorization.expires_at && authorization.expires_at < GuardianUtils.timestamp do
  ...
  defp create_user(auth, repo) do
    ...
    changeset = User.registration_changeset(%User{}, scrub(%{email: auth.info.email, name: name}))
        ...
        authorization ->
          case auth.credentials.other.password do
            pass when is_binary(pass) ->
              if Bcrypt.checkpw(auth.credentials.other.password, authorization.token) do
                authorization
              else
